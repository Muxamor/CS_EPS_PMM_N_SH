...
/** @brief  Get temperature from INA238 sensor.
	@param  *pam_ptr - pointer to struct which contain all information about PAM.
	@param  *I2Cx - pointer to I2C controller, where x is a number (e.x., I2C1, I2C2 etc.).
	@param  tmp1075_addr - I2C sensor address
	@param  i2c_mux_addr - I2C MUX address
	...
	@retval 0 - SUCCESS, -1 - ERROR_N.
*/
ErrorStatus PAM_INA238_Get_Temperature( _PAM *pam_ptr, uint8_t num_pwr_ch, float *temp_value){
	...
	if( error_I2C == SUCCESS ){
		...
		while( ( error_I2C != SUCCESS ) && ( i < pam_i2c_attempt_conn ) ){///Read temperature.
			...
			error_I2C = INA238_Get_Temperature_float(pam_table.I2Cx_PORT, pam_table.I2C_addr_PWR_Mon, temp_value);
...
ErrorStatus PAM_Get_PWR_CH_IN_I_V_P( _PAM *pam_ptr, uint8_t num_pwr_ch){
	...
	if( error_I2C == SUCCESS ){
		...
		while( ( error_I2C != SUCCESS ) && ( i < pam_i2c_attempt_conn ) ){///Read temperature.
			...
			error_I2C = INA238_Get_I_V_P_float( pam_table.I2Cx_PORT, pam_table.I2C_addr_PWR_Mon, pam_table.PWR_Mon_Max_Current_float, &val_current, &val_bus_voltage, &val_power );
	...
	pam_ptr->PWR_IN_Channel[num_pwr_ch].Voltage_val = (uint16_t)val_bus_voltage;
	pam_ptr->PWR_IN_Channel[num_pwr_ch].Current_val = (int16_t)val_current;
	pam_ptr->PWR_IN_Channel[num_pwr_ch].Power_val = (uint32_t)val_power;
...
//Comment this code for board PAM2.1
