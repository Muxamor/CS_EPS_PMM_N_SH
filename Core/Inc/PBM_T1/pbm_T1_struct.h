...
typedef struct  // Structure for PBM data
{
	...
	uint8_t DchgEnableBit :1; 				// Discharge Enable Bit must be set zero to allow the ON state. // Need to save in flash.
	uint8_t ChgEnableBit :1; 				// Charge Enable Bit must be set zero to allow the ON state. // Need to save in flash.
	...
	int16_t VoltageHi;						// Register voltage of high battery (on 8.4V) in mV.
	int16_t VoltageLo;						// Register voltage of lower battery (on GND) in mV.
	int16_t Current;						// Register current level over the preceding 351ms in mA.
	int16_t AverageCurrent;					// Register average current level over the preceding ?s in mA.
	...
	int16_t AbcoluteCapacity_mAh;			// Register accumulate charge of battery's in mAh.
...
typedef struct  // Structure for PBM data
{
	_T1_Branch_Data Branch[2]; 				// All branch data struct.
...
/*	uint8_t Branch_1_OVP :1; 				// Overvoltage faults.
	...
	uint8_t Branch_1_RelativeCapacity_Perc;	// Register active relative Capacity in %.
	int16_t Branch_1_AbcoluteCapacity_mAh;	// Register accumulate charge of battery's in mAh.
	...
	uint8_t Branch_2_RelativeCapacity_Perc;	// Register active relative Capacity in %.
	int16_t Branch_2_AbcoluteCapacity_mAh;	// Register accumulate charge of battery's in mAh.
	...
	uint8_t PCA9534_Emerg_Chrg_1 :1; 		// PCA9534 port №3. Show state emergency charge branch №1.
	uint8_t PCA9534_Emerg_Chrg_2 :1; 		// PCA9534 port №6. Show state emergency charge branch №2.
	...
	uint16_t Error_Emerg_Chrg_1 :1; 		// Register error emergency charge branch №1.
	uint16_t Error_Emerg_Chrg_2 :1; 		// Register error emergency charge branch №2.
