...
struct CAN_IVar5{
    ...
    // -------------------  Beacon  ------------------ //
	uint8_t CAN_Beacon_PMM_Switch_Active_CPU_double;     	        //+0       |  const = 0x                 	|  Переключатель основной/резервный МАЯК
	int8_t  CAN_Beacon_PMM_Temp_Sensor_double;                      //+1       |  const = 0x                 	|  PMM датчик 1
	uint16_t CAN_Beacon_EPS_Main_Bus_Voltage_double;		        //+2,3    |  const = 0x                 	|  Напряжение внутренней силовой шины СЭС
	uint16_t CAN_Beacon_EPS_CubeSat_Total_Cons_Power_double;        //+4,5    |  const = 0x                 	|  Полная потребляемая мощность
	uint16_t CAN_Beacon_EPS_CubeSat_SubSystem_Cons_Power_double;    //+6,7    |  const = 0x                 	|  Потребляемая мощность полезной нагрузкой и подсистемами КА
	uint16_t CAN_Beacon_EPS_Total_PAM_Generation_Power_double;      //+8,9    |  const = 0x                 	|  Полная вырабатываемая мощность СБ
	uint8_t  CAN_Beacon_PAM_In_PwrCh_ID_State_BitsMask_double;      //+10       |  const = 0x                 	|  Ideal Diode
	int8_t  CAN_Beacon_SP_TM_Ch1_Median_Temp_double;                //+11       |  const = 0x                 	|  Медианная температура панели Y+
	int8_t  CAN_Beacon_SP_TM_Ch2_Median_Temp_double;                //+12       |  const = 0x                 	|  Медианная температура панели X-
	int8_t  CAN_Beacon_SP_TM_Ch3_Median_Temp_double;                //+13       |  const = 0x                 	|  Медианная температура панели Y-
	int8_t  CAN_Beacon_SP_TM_Ch4_Median_Temp_double;                //+14       |  const = 0x                 	|  Медианная температура панели X+
	int8_t  CAN_Beacon_SP_TM_Ch5_Median_Temp_double; 				//15Нет в Mule +15       |  const = 0x                 	|  Медианная температура откидной панели +Y
	int8_t  CAN_Beacon_SP_TM_Ch6_Median_Temp_double;  				//16Нет в Mule +16       |  const = 0x                 	|  Медианная температура откидной панели -Y
	int16_t CAN_Beacon_PBM_Total_PBM_Char_Dischar_Power_double;     //+17,18    |  const = 0x                 	|  Полная мощность зарядки/разрядки АБ
	uint16_t CAN_Beacon_PBM_Total_Heating_Elements_Power_double;	//+19,20    |  const = 0x                	|  Полная мощность нагревателей АБ
	uint16_t CAN_Beacon_PBM_Total_Battery_Level_mAh_double;		    //+21,22    |  const = 0x                 	|  Полный уровень заряда АБ в mAh
	uint8_t  CAN_Beacon_PBM_Heating_Element_State_BitMask_double;   //+23       |  const = 0x                 	|  Текущее состояние Термостата
	uint8_t  CAN_Beacon_PBM_Error_Status_BitsMask_BEACON_double[3]; //+24,25,26 |  const = 0x                 	|  Статусы отказов (ошибок)АБ, битовая маска (МАЯК)
	uint8_t  CAN_Beacon_EPS_Error_Status_BitsMask_BEACON_double[2]; //+27,28    |  const = 0x                 	|  Статус отказов элементов СЭС, битовая маска (МАЯК)
	uint8_t Protaction_GAP_Beacon[35];                              //29..             	//Total 29 byte
    ...
    int8_t  CAN_Beacon_SP_TM_Ch5_Median_Temp;     					//79Нет в Mule +15       |  const = 0x                 	|  Медианная температура откидной панели +Y
    int8_t  CAN_Beacon_SP_TM_Ch6_Median_Temp;     					//80Нет в Mule +16       |  const = 0x                 	|  Медианная температура откидной панели -Y
